...
export default function Home() {
  ...
  const todayAppointments = useMemo(() => {
    return appointments.filter((item) => isToday(item.start_time));
  }, [appointments]);

  const activeProfessionals = useMemo(() => {
    return professionals.filter((item) => item.active !== false).length;
  }, [professionals]);
  ...
  const cancelledAppointments = useMemo(() => {
    return appointments.filter((item) => item.status === "cancelled").length;
  }, [appointments]);
        ...
        <div className="appointments-list">
          {loading ? (
            ...
            nextAppointments.map((item) => {
              ...
              const serviceName = item.service_name || "Serviço não informado";
